...
class Note extends FlxSprite
{
	...
	public var noNameNote:Bool = false;
	public var timeNote:Bool = false;
	public var yOffset:Int = 0;

	public var rating:String = "shit";

	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?damageNote = false, ?timeNote = false, ?noNameNote = false)
	{
		super();
		...
		switch (noteTypeCheck)
		{
			case 'pixel':
				...
				updateHitbox();
				case 'darkz':
					frames = Paths.getSparrowAtlas('dark_notes');
					...
					antialiasing = true;
			default:
				if (!damageNote && !timeNote && !noNameNote) {
					...
					frames = Paths.getSparrowAtlas('notes/KnifeInput', 'fumo');
					...
					frames = Paths.getSparrowAtlas('notes/time', 'fumo');
					animation.addByPrefix('greenScroll', 'green');
					animation.addByPrefix('redScroll', 'red');
					animation.addByPrefix('blueScroll', 'blue');
					animation.addByPrefix('purpleScroll', 'purple');
					...
					trace("e");
					frames = Paths.getSparrowAtlas('notes/NoNameInput', 'fumo');
					animation.addByPrefix('greenScroll', 'green');
					animation.addByPrefix('redScroll', 'red');
					animation.addByPrefix('blueScroll', 'blue');
					animation.addByPrefix('purpleScroll', 'purple');
	...
	override function update(elapsed:Float)
	{
		...
		if (mustPress)
		{
			PlayState.instance.notes.forEachAlive(function(daNote:Note) {
				if (daNote != this && overlaps(daNote)) {
					yOffset = 10000;
				}
			});

			if (PlayState.timestop && y > 200 - yOffset && y < FlxG.height - 200 + yOffset) {
				strumTime += elapsed * 1000;
			}
